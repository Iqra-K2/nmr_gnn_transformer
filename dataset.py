...
class GraphDataset():
    ...
    def __getitem__(self, idx):
        ...
        # Spectrum input: aggregate chemical shifts (input to Transformer)
        if self.target == '1H':
            # Flatten and stack the 1H shifts per atom (1D array of floats)
            spectrum = np.hstack(
                [np.hstack(s) for s in self.shift[idx][self.mask[self.n_csum[idx]:self.n_csum[idx + 1]]]])
        else:
            # For 13C, shifts are single values per atom
            spectrum = self.shift[self.n_csum[idx]:self.n_csum[idx + 1]].astype(float)

        # Pad or truncate to fixed input length if needed
        spectrum = torch.tensor(spectrum, dtype=torch.float)

        # SMILES as output (can tokenize later in dataloader or collate_fn)
        smiles = self.smi[idx]

        return g, spectrum, smiles
